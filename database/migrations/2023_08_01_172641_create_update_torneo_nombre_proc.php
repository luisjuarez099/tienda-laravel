...
return new class extends Migration
{
    ...
    public function up()
    {
        DB::unprepared("CREATE DEFINER=`equipo1`@`%` PROCEDURE `update_torneo_nombre`(
...
BEGIN
    ...
    SELECT is_torneo_already_created(NEW_NOMBRE, DEPORTE, INICIO, CENTRO, INSTALACION) INTO torneo_exists;

    -- si el torneo no existe ejecutamos el update
    IF torneo_exists < 1 THEN
        UPDATE torneo SET nombre = NEW_NOMBRE WHERE idtorneo = torneo_id;
    END IF;
...
END");
    ...
    {
        DB::unprepared("DROP PROCEDURE IF EXISTS update_torneo_nombre");
    }
};
